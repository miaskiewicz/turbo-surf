...
impl TlsConnectorBuilder {
        ...
        T: Into<Cow<'a, TlsOptions>>,
    {
        ...
        // Set requested trust anchors — the `trust_anchors` ClientHello extension
        // (codepoint 0xCA34, draft-ietf-tls-trust-anchor-ids) that Chrome 152+
        // sends. turbo-surf patch (not in upstream wreq): btls-sys does not bind
        // `SSL_CTX_set1_requested_trust_anchors`, so we declare and call it here on
        // the connector's raw `SSL_CTX` (the symbol is provided by btls-sys's
        // bundled BoringSSL). An empty list still emits the extension, which is all
        // we need to match Chrome's JA4 (`t13d1517h2`).
        #[allow(unsafe_code)]
        if let Some(ref ids) = opts.trust_anchors {
            // int SSL_CTX_set1_requested_trust_anchors(SSL_CTX*, const uint8_t*, size_t)
            unsafe extern "C" {
