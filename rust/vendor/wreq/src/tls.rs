...
pub struct TlsOptions {
    ...
    /// Requested trust anchors, sent as the `trust_anchors` ClientHello extension
    /// (codepoint 0xCA34, draft-ietf-tls-trust-anchor-ids). The value is a
    /// wire-format list of trust anchor IDs (a series of 8-bit length-prefixed
    /// strings). Per BoringSSL, an **empty** list (`Some` of zero bytes) still
    /// emits the extension — which is how Chrome 152+ advertises support for the
    /// retry flow without naming anchors. `None` omits the extension entirely.
    ///
    /// turbo-surf patch (not in upstream wreq): applied via
    /// `SSL_CTX_set1_requested_trust_anchors` in `tls/conn.rs`.
    ///
    /// **Default:** `None`
    pub trust_anchors: Option<Bytes>,
}
...
impl TlsOptionsBuilder {
    ...
    /// Sets the requested trust anchors (see [`TlsOptions::trust_anchors`]).
    ///
    /// Pass an empty slice to emit the `trust_anchors` extension without naming
    /// any anchors — matching Chrome 152+ (turbo-surf patch, not upstream wreq).
    #[inline]
